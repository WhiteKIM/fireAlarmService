...
class Model:
    def __init__(self, name, index, cam, x, y, z, w):
        self.name = name
        self.index = index
        self.cam = cam
        self.x = x
    ...
    def printLocation(self):
        print('name '+str(self.name)+str(self.index)+' '+'Location XYZW : '+str(self.x)+' '+str(self.y)+' '+str(self.z)+' '+str(self.w))
        ...
        while True: # while video is running
            ...
            for track in self.tracker.tracks:  # update new findings AKA tracks
                ...
                if(check == False):
                    ...
                    modelList.append(Model(class_name ,str(track.track_id), self.video ,int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])))
